...
| `sha`        | SHA of commit to update status on, defaults to context github.sha. *If using `on: pull_request` use `github.event.pull_request.head.sha`* | context github.sha                   |
